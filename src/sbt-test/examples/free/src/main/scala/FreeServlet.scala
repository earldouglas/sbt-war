...
class FreeServlet extends HttpServlet {
  ...
  implicit val responseMonad: Monad[Either[WebAppErr, ?]] =
    new Monad[Either[WebAppErr, ?]] {
      ...
      def bind[A, B](fa: Either[WebAppErr, ?][A])(f: A => Either[WebAppErr, ?][B]): Either[WebAppErr, ?][B] =
        fa match {
          case Left(e) => Left(e)
          case Right(a) => f(a)
        }
  ...
  def interpreter(req: HttpServletRequest): WebAppOp ~> Either[WebAppErr, ?] =
      ...
      def apply[A](c: WebAppOp[A]): Either[WebAppErr, ?][A] =
        try {
          c match {
            ...
            case RequestHeader(name) => Option(req.getHeader(name)).map(Right(_)).getOrElse(Left(MissingHeader(name)))
            case RequestParam(name)  => Option(req.getParameter(name)).map(Right(_)).getOrElse(Left(MissingParameter(name)))
            case Error(e)            => Left(e)
          }
        } catch {
          case t: Throwable =>
