...
object WebPlugin extends Plugin {
	...
	val webSettings = Seq(
		...
		managedClasspath in Configurations.RuntimeInternal <<= (classpathTypes, update) map { (ct, report) => Classpaths.managedJars(Configurations.RuntimeInternal, ct, report)},
